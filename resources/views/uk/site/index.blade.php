...
@section('content')
    @if(count($main['aboutme'])>0)
        <section class="section summary-section">
            <h2 class="section-title">
                <i class="fas fa-user"></i>Про мене</h2>
            <div class="summary">
    ...
    @if(count($main['experiences'])>0)
        <section class="section summary-section">
            <h2 class="section-title">
                <i class="fas fa-building"></i>Досвіди роботи</h2>
            @foreach($main['experiences'] as $item)
                <div class="item">
                    <div class="meta">
                        <div class="upper-row">
                            <h3 class="job-title">{{ $item->titleuk }}</h3>
                            <div class="time">{{ $item->timeuk }}</div>
                        </div>
                        <div class="company">{{ $item->companyuk }}</div>
                    </div>
                    <div class="details">
                        <p>Cтек технологій: {{ $item->textuk }}</p>
                    </div>
                </div>
            @endforeach
        </section>
    ...
    @if(count($main['skills'])>0)
        <section class="skills-section section">
            <h2 class="section-title">
                <i class="fas fa-list"></i>Навички та вміння</h2>
            <div class="skillset">
    ...
    @endif
    @if(count($main['projects'])>0)
        <section class="section summary-section">
            <h2 class="section-title">
                <i class="fas fa-briefcase"></i>Проекти</h2>
            <div class="summary">
                <div class="row">
